...
def kube_command_obj(cluster_values):
  ...
  kube_commands = {
    ...
    "replace_deployment": v2.replace_namespaced_deployment ## https://github.com/kubernetes-client/python/blob/master/kubernetes/docs/AppsV1Api.md#replace_namespaced_deployment
  }
...
def update_deployment(src, kube_command, git_tag_name):
  ...
  if src in deployment_list:
    try:
      ...
      message = kube_command.get("replace_deployment")(name=deployment_name, namespace=deployment_namespace, body=new_deployment_obj)
      print(message.status.conditions[1].message)
      good_answers = [ "is progressing", "Deployment has minimum availability", "has successfully progressed" ]
      bad_answers = [ "has timed out progressing" ]
      for ans in good_answers:
        if ans in message.status.conditions[1].message: ## here the deployment will be replaced
          return f"{src} deployment replaced"
      else:
        raise Exception(f"{src} deployment failed")
    except Exception as e:
      raise Exception(e)
...
def update_version(versionArr, commit_message):
  ...
  for element in commit_message.split(" "):
        if element.find("#") != -1:
          if element == "#minor": # minor update by 2 is required
              ...
              return versionArr
          elif element == "#major": # major update by 2 is required
              versionArr[2] = str(int(0))
              versionArr[1] = str(int(0))
              versionArr[0] = str(int(versionArr[0]) + 2)
              return versionArr
          elif element == "#patch": # patch update by 2 is required
              versionArr[2] = str(int(versionArr[2]) + 2)
              return versionArr
  return versionArr

def increment_version(package):
  '''
  this function increase the version in package json and write it back
  '''

  commit_message = os.environ['COMMIT_MESSAGE']
  os.chdir(f'{path}/packages/{package}')
  ...
  versionArr = update_version(versionArr, commit_message)

  with open('package.json') as f:
    data = json.load(f)
  data['version'] = '.'.join(versionArr)
